...
public class ImageAdjustorWidget extends JPanel implements ChangeListener {
	...
	public ImageAdjustorWidget(Picture picture) {
		...
		// adds a slider panel and a pictureView
		...
		//sets the original value, the increment values, and adds a change listener. Has increments of 1
		...
		// does the same as Blur. has increments of 25
		...
		// does the same as blur and saturation, but has increments of 25
		...
		// dimensions of the slider panel
	...
	public Picture setBlur(Picture picture) {
		...
		for (int i = 0; i < picture.getWidth(); i++) {
			for (int j = 0; j < picture.getHeight(); j++) {
				...
				avgBlue = 0.0;
				count = 0;
				//averages the values by going through the for loop. Must start at a negative value
	...
	public Picture setBrightness(Picture picture){
	  ...
	  Picture brightenedPic = new PictureImpl(picture.getWidth(), picture.getHeight());
		...
		for (int i = 0; i < picture.getWidth(); i++) {
		  ...
		  //uses the darkened and lihtened methods from A3 in order to create a light or dark picture
	  ...
	  for (int i = 0; i < picture.getWidth(); i++){
	     ...
	     if( picture.getPixel(i, j).getRed() == 0 && picture.getPixel(i, j).getGreen() == 0 && picture.getPixel(i, j).getBlue() == 0){
		  ...
		  if(saturationValue<= 0){
		    ...
		    // has to have two types of. One if it is greater than 0 and one if it is less than 0
	...
	public void stateChanged(ChangeEvent e) {
         // allows all of the states to happen if the slider is adjusting. If the slider is not adjusting nothing will happen
